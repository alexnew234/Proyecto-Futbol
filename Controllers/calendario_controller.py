...
class CalendarioController:
    ...
    def _is_english(self):
        return bool(self.main_controller and getattr(self.main_controller, "current_language", "es") == "en")
    ...
    def setup_ui(self):
        ...
        # Tamaño y color como en la captura de referencia
        self.reloj_header.setFixedSize(220, 58)
    ...
    def cargar_calendario(self):
        # Designer puede restaurar columnas al retraducir la UI.
        self.tree_widget.setColumnCount(1)
        self.tree_widget.setHeaderHidden(True)
        self.tree_widget.setTextElideMode(Qt.TextElideMode.ElideNone)
        if hasattr(self.tree_widget.header(), "setStretchLastSection"):
    ...
    def editar_partido(self, item, column):
        ...
        form_layout.addWidget(QLabel(
            self._tr("Goles {equipo}:", "Goals {equipo}:").format(equipo=local),
        ...
        form_layout.addWidget(QLabel(
            self._tr("Goles {equipo}:", "Goals {equipo}:").format(equipo=visit),
            font=QFont("Arial", 12, QFont.Bold)
        ))
        ...
        lh = QHBoxLayout()
        v1 = QVBoxLayout()
        v1.addWidget(QLabel(self._tr("Fecha:", "Date:")))
        date_edit = QDateEdit()
        ...
        v2 = QVBoxLayout()
        v2.addWidget(QLabel(self._tr("Hora:", "Time:")))
        ...
        form_layout.addWidget(QLabel(self._tr("Árbitro asignado:", "Assigned referee:")))
        combo_arb = QComboBox()
        ...
        combo_arb.addItem(self._tr("Sin asignar", "Unassigned"), None)
        ...
        btn_players = QPushButton(self._tr("Gestionar Goles y Tarjetas", "Manage Goals and Cards"))
        ...
        lbl_reloj = QLabel(self._tr("⏱ CRONÓMETRO", "⏱ CHRONOMETER"), font=QFont("Arial", 14, QFont.Bold))
        ...
        self.lbl_aviso_reloj = QLabel(self._tr("Esperando señal del reloj...", "Waiting for clock signal..."))
        ...
        def mostrar_aviso_reloj(mensaje, mostrar_popup=False):
            texto = self._tr("AVISO: {mensaje}", "NOTICE: {mensaje}").format(mensaje=mensaje)
        ...
        btn_iniciar = QPushButton(self._tr("▶ Iniciar", "▶ Start"))
        ...
        btn_pausar = QPushButton(self._tr("⏸ Pausar", "⏸ Pause"))
        ...
        btn_reiniciar = QPushButton(self._tr("↺ Reiniciar", "↺ Reset"))
        ...
        btn_save = QPushButton(self._tr("GUARDAR RESULTADO DEL PARTIDO", "SAVE MATCH RESULT"))
        ...
        def guardar():
            ...
            if qu.exec():
                ...
                QMessageBox.information(
                    ...
                    self._tr("Resultado actualizado.\nDuracion del partido: {duracion}", "Match result updated.\nMatch duration: {duracion}").format(
    ...
    def gestionar_jugadores(self, id_local, id_visitante):
        dialog = QDialog(self.main_view)
        dialog.setWindowTitle(self._tr("Estadísticas de Jugadores", "Player Statistics"))
        ...
        for eid, tipo_eq in equipos:
            ...
            tabla.setHorizontalHeaderLabels([
                self._tr("Nombre del Jugador", "Player Name"),
                self._tr("Goles (+)", "Goals (+)"),
                self._tr("Amarillas (+)", "Yellow (+)"),
                self._tr("Rojas (+)", "Red (+)")
            ])
            tabla.setStyleSheet("""
            ...
            while q.next():
                ...
                pid = q.value(0)
                nombre = q.value(1)
                txt = self._tr("{nombre} (Total: {goles}G)", "{nombre} (Total: {goles}G)").format(
                    nombre=nombre,
                    goles=q.value(2)
                )
        ...
        btn_guardar = QPushButton(self._tr("CONFIRMAR Y ACTUALIZAR ESTADÍSTICAS", "CONFIRM AND UPDATE STATS"))
    ...
    def nueva_temporada(self):
        if QMessageBox.question(
            self.main_view,
            self._tr("Reiniciar", "Reset"),
            self._tr("¿Borrar TODO?", "Delete EVERYTHING?"),
            QMessageBox.Yes | QMessageBox.No
        ) == QMessageBox.Yes:
            ...
            QMessageBox.information(
                self.main_view,
                self._tr("Listo", "Done"),
                self._tr("Temporada reiniciada.", "Season reset.")
            )

    def retranslate_ui(self):
        """Actualiza textos de la vista de calendario al cambiar idioma."""
        self.btn_generar_ronda.setText(self._tr("Generar Siguiente Ronda", "Generate Next Round"))
        self.btn_clasificacion.setText(self._tr("Ver Clasificación", "View Standings"))
        self.btn_nueva_temporada.setText(self._tr("Nueva Temporada", "New Season"))
        self.btn_volver.setText(self._tr("Volver al Calendario", "Back to Calendar"))
        if hasattr(self.main_view.ui, "label_titulo"):
            self.main_view.ui.label_titulo.setText(
                "<html><head/><body><p align=\"center\"><span style=\" font-size:20pt;\">"
                + self._tr("Calendario del torneo", "Tournament Calendar")
                + "</span></p></body></html>"
            )
        self.cargar_calendario()
